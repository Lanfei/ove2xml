...
int NoteUtility::NoteTypeToTick(NoteType type, int quarter) {
...
void MusicData::setXmlVoice(unsigned int voice) {
	xmlVoice_ = voice;
}

unsigned int MusicData::getXmlVoice() const {
	return xmlVoice_;
}
...
void OveSong::addMeasure(Measure* measure) {
...
void OveSong::addMeasureData(MeasureData* measureData) {
	measureDatas_.push_back(measureData);
}
...
bool TrackParse::parse() {
	...
	for( i=0; i<8; ++i ) {
		...
		// channel
		if( !readBuffer(placeHolder, 1) ) { return false; }
		voice->setChannel(placeHolder.toUnsignedInt());

		// volume
		if( !readBuffer(placeHolder, 1) ) { return false; }
		voice->setVolume(placeHolder.toInt());

		// pitch shift
		if( !readBuffer(placeHolder, 1) ) { return false; }
		voice->setPitchShift(placeHolder.toInt());

		// pan
		if( !readBuffer(placeHolder, 1) ) { return false; }
                voice->setPan(placeHolder.toInt() + 64);

		if( !jump(6) ) { return false; }

		// patch
		if( !readBuffer(placeHolder, 1) ) { return false; }
		voice->setPatch(placeHolder.toInt());
...
bool BarsParse::parseNoteRest(MeasureData* measureData, int length, BdatType type) {
	...
	if( type == Bdat_Rest ) {
		Note* rest = new Note();
		container->addNoteRest(rest);
		rest->setIsRest(true);

		// line
		if( !readBuffer(placeHolder, 1) ) { return false; }
		rest->setLine(placeHolder.toInt());
...
void OveOrganizer::organizePairElement(
	...
	if( data->getMusicDataType() == MusicData_Tuplet ){
		...
		int tupletTick = NoteUtility::NoteTypeToTick(tuplet->getNoteType(), ove_->getQuarter())*tuplet->getSpace();
